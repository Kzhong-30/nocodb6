...
import { isVersionConflictError } from '~/utils/errorUtils'
...
export function useData(args: {
  ...
  async function updateRowProperty(
    ...
    try {
      ...
      const lastModifiedTimeCol = metaValue?.columns?.find((col: ColumnType) => col.uidt === UITypes.LastModifiedTime)
      const oldLastModifiedTime = lastModifiedTimeCol?.title ? toUpdate.oldRow?.[lastModifiedTimeCol.title] : null
      ...
      if (oldLastModifiedTime !== undefined && oldLastModifiedTime !== null) {
        updateData.__nc_old_updated_at = oldLastModifiedTime
      }
      ...
      if (!undo) {
        ...
        Object.assign(
          toUpdate.row,
          metaValue!.columns!.reduce<Record<string, any>>(
            (acc: Record<string, any>, col: ColumnType) => {
              if (
      ...
      if (isVersionConflictError(e)) {
        ...
        const errorMessage = errorData?.message || (await extractSdkResponseErrorMsg(e)) || t('msg.error.versionConflict')

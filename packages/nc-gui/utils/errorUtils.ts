...
/**
 * Checks if the error is a version conflict (optimistic lock violation)
 * @param e - Error object
 * @returns true if it's a version conflict error
 */
export function isVersionConflictError(e: Error & { response?: any }): boolean {
  if (!e?.response?.data) return false

  const errorData = e.response.data
  return (
    errorData.error === NcErrorType.ERR_VERSION_CONFLICT ||
    (errorData.message ?? errorData.msg)?.includes('version conflict') ||
    (errorData.message ?? errorData.msg)?.includes('modified by another user') ||
    (errorData.message ?? errorData.msg)?.includes('refresh and try again')
  )
}
